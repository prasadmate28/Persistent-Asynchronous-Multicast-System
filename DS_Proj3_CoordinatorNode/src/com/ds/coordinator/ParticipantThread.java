...
public class ParticipantThread implements Runnable {
	...
	public ParticipantThread(Socket c_socket) throws IOException {
		...
		inpStream = new DataInputStream(c_socket.getInputStream()); // data i/o
																	// stream on
																	// server
																	// port
		outStream = new DataOutputStream(c_socket.getOutputStream()); // data
																		// i/o
																		// stream
																		// on
																		// server
	...
	public void run() {
		...
		try {
				...
				if (!userCmd.equals("deregister")) {
					...
					case "register":
						if (participant.connectionStatus == null) {
							participant.setPortNo(Integer.parseInt(readFromInpStream()));// set
																	// listener
																	// port
																	// number
							if (c_util.register()) {// true if participant id is
													// unique
						...
						break;
					/*
					 * case "deregister": if(participant.connectionStatus !=
					 * null){ c_util.deregister();
					 * writeToOutStream("Deregistration successful"); return;
					 * }else{ writeToOutStream("Deregistration unsuccessful"); }
					 * return;
					 */
					case "reconnect":
						if (participant.connectionStatus.equals(DISCONNECTED)) {//"Disconnected"
							c_util.reconnect();
						...
						break;
					case "disconnect":
						if (participant.connectionStatus.equals(REGISTERED)) {//"Registered"
							c_util.disconnect();
			...
			System.out.println("Exception in executing commands :::");
			e.printStackTrace();
		}
	}
