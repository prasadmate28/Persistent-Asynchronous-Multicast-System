...
public class MCCommandDispatcher implements Runnable {
	...
	public void run() {
		...
		try{
			...
			System.out.println("Exception in creating command dispatcher thread:: "+e.getMessage());
			//e.printStackTrace();
		}
	...
	private boolean executeCommandDispatch(String userCmd) {
		...
		try{
			...
			switch (command.get(0)){
				...
				case "register":
						if(!Participant.participantConnectionStatus){//not registered
	...
	private void makeMCReceiver(int listenerPortno,String logFileName) {
		...
		try{
			...
			System.out.println("Exception in creating MC read thread"+e.getMessage());
			//e.printStackTrace();
		}
	}
